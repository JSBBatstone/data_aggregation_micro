...
export class AggregationService {
  ...
  async getAggregatedDataByUserId(userId: string) {
    ...
    const result = await this.transactionRepository
      ...
      .addSelect(
        `ROUND(SUM(CASE WHEN transaction.type = 'payout' THEN transaction.amount ELSE 0 END) * ${exchangeRate}, 2)`,
        'payout',
      )
      .addSelect(
        `ROUND(SUM(CASE WHEN transaction.type = 'paid_out' THEN transaction.amount * ${exchangeRate} ELSE 0 END), 2)`,
        'paid_out',
      )
      .addSelect(
